...
const DailySummaryHistory: React.FC<DailySummaryHistoryProps> = ({ isOpen, onClose }) => {
  ...
  const loadData = () => {
    const profileSaved = localStorage.getItem(USER_PROFILE_KEY);
    if (profileSaved) setUserProfile(JSON.parse(profileSaved));

    const summariesSaved = localStorage.getItem(DAILY_SUMMARIES_KEY);
    if (summariesSaved) {
  ...
  return (
    <div className="fixed inset-0 bg-[var(--modal-overlay-bg)] backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[var(--component-bg)] backdrop-blur-xl rounded-2xl ring-1 ring-[var(--glass-border)] w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-4 border-b border-[var(--glass-border)]"><div className="flex items-center gap-3"><CalendarIcon className="w-6 h-6 text-indigo-400" /><h2 className="text-xl font-bold text-[var(--text-primary)]">Daily Summary History</h2></div><button onClick={onClose} className="p-1 text-[var(--icon-color)] hover:text-[var(--text-primary)]"><CloseIcon className="w-6 h-6" /></button></header>
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {totalDays > 0 && (
            <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 backdrop-blur-lg rounded-xl p-6 ring-1 ring-indigo-500/30">
              ...
              <div className="grid grid-cols-2 text-center bg-white/5 rounded-lg p-3"><p className="text-2xl font-bold text-[var(--text-primary)]">{totalDays}</p><p className="text-xs text-[var(--text-secondary)]">Days Logged</p><p className="text-2xl font-bold text-[var(--text-primary)]"><AnimatedNumber value={avgCalories} /></p><p className="text-xs text-[var(--text-secondary)]">Avg Calories</p></div>
              ...
              {aiInsight && <div className="mt-4 bg-purple-500/10 border border-purple-500/30 rounded-lg p-4"><h4 className="font-semibold text-purple-300 mb-2">✨ AI Insights</h4><p className="text-[var(--text-secondary)] text-sm whitespace-pre-line">{aiInsight}</p></div>}
            </div>
          )}
          <div className="space-y-4">
            {summaries.map((s) => (<div key={s.date} className="bg-[var(--component-bg)] backdrop-blur-lg rounded-xl p-5 ring-1 ring-[var(--glass-border)]" style={{boxShadow: 'var(--card-shadow)'}}><div className="flex items-center justify-between mb-4"><div><h3 className="text-lg font-bold text-[var(--text-primary)]">{formatDate(s.date)}</h3><p className="text-sm text-[var(--text-secondary)]">{s.date} • {s.mealsLogged} meals</p></div><div className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(s.goalsMet)}`}>{getStatusText(s.goalsMet)}</div></div>{/* Progress Bars */}</div>))}
          </div>
          {summaries.length === 0 && <div className="text-center py-12"><CalendarIcon className="w-16 h-16 text-zinc-600 mx-auto mb-4" /><p className="text-[var(--text-secondary)]">No data for this period</p></div>}
        </div>
      </div>
    </div>
  );
